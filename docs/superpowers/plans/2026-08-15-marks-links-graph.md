...
GW_DATABASE_URL="sqlite:///tmp/mig-check.db" cargo run -p gw-api -- check
```
Expected: exits 0. A migration that only works on an empty database is a migration that fails in production.
